    ...
    ens.methods.resolver(node).call().then(function(resolverAddress) {
        if (resolverAddress !== 0) {
            ...
            resolverContract.methods.addr(node).call().then(function(result) {
                if (result !== 0 && callback) callback(result);
            });
        }
    });
}
    ...
    ens.methods.resolver(node).call(function(error, resolverAddress) {
        ...
        if (resolverAddress !== 0) {
            ...
            resolverContract.methods.name(node, function(error, result) {
                if (!error && result !== 0 && callback) callback(result);
            });
        }
    });
}
    ...
    webu.huc.isSyncing(function(err, syncing) {
        if (!err && !syncing) {
            webu.huc.getCode(ensAddress, function(err, code) {
                if (!err && code.length > 2) TemplateVar.set(template, 'ensAvailable', true);
            });
        }
    });
});
...
Template.dapp_addressInput.onRendered(function() {
    if (this.data) this.$('input').trigger('change');
});
...
Template.dapp_addressInput.helpers({
    ...
    i18nText            : function() {
        if (typeof TAPi18n === 'undefined' || TAPi18n.__('elements.checksumAlert') === 'elements.checksumAlert') {
            return 'This address looks valid, but it doesn\'t have some security features that will protect you against typos, ' +
                'so double check you have the right one. If provided, check if the security icon  matches.';
        }
    ...
    'input input, keyup input': function(e, template) {
        ...
        if (webu.isAddress(value) || _.isEmpty(value)) {
            ...
            if (!_.isEmpty(value)) {
                ...
                TemplateVar.set('isChecksum', webu.isChecksumAddress(value));
